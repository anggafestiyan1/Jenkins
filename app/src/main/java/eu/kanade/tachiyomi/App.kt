...
class App : Application(), DefaultLifecycleObserver, SingletonImageLoader.Factory {
    ...
    override fun onCreate() {
        ...
        seedDefaultExtensionRepos(scope)

        initializeMigrator()
    }

    private fun seedDefaultExtensionRepos(scope: androidx.lifecycle.LifecycleCoroutineScope) {
        scope.launch(Dispatchers.IO) {
            try {
                val mangaCount = Injekt.get<GetMangaExtensionRepoCount>().subscribe().first()
                if (mangaCount == 0) {
                    Injekt.get<CreateMangaExtensionRepo>().await(
                        "https://raw.githubusercontent.com/keiyoushi/extensions/repo/index.min.json",
                    )
                }
                val animeCount = Injekt.get<GetAnimeExtensionRepoCount>().subscribe().first()
                if (animeCount == 0) {
                    Injekt.get<CreateAnimeExtensionRepo>().await(
                        "https://raw.githubusercontent.com/aniyomiorg/aniyomi-extensions/repo/index.min.json",
                    )
                }
            } catch (e: Exception) {
                logcat(LogPriority.WARN, e) { "Failed to seed default extension repos" }
            }
        }
    }
