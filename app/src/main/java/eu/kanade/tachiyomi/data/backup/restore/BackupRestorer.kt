...
class BackupRestorer(
    ...
    /**
     * Source IDs referenced by the backup but not installed locally.
     * Surfaced to the user so they can install the missing extensions and
     * re-run the restore to fully recover entries from those sources.
     */
    private val missingMangaSourceIds = mutableSetOf<Long>()
    private val missingAnimeSourceIds = mutableSetOf<Long>()
    ...
    private suspend fun restoreFromFile(uri: Uri, options: RestoreOptions) {
        ...
        mangaSourceMapping = backup.backupSources.associate { it.sourceId to it.name }
    ...
    private fun CoroutineScope.restoreAnime(
        ...
        animeRestorer.sortByNew(backupAnimes)
            .forEach {
                ...
                if (animeSourceManager.get(it.source) == null) {
                    missingAnimeSourceIds.add(it.source)
                }
    ...
    private fun CoroutineScope.restoreManga(
        ...
        mangaRestorer.sortByNew(backupMangas)
            .forEach {
                ...
                if (mangaSourceManager.get(it.source) == null) {
                    missingMangaSourceIds.add(it.source)
                }
    ...
    /**
     * Appends a summary of source IDs the backup referenced but that aren't installed
     * locally. Surfaced via the existing error log so users can install the missing
     * extensions and re-run the restore to recover entries from those sources.
     */
    private fun reportMissingSources() {
        if (missingMangaSourceIds.isEmpty() && missingAnimeSourceIds.isEmpty()) return

        val now = Date()
        errors.add(
            now to "Some sources were not found. Install the missing extensions, " +
                "then restart restore to recover entries from those sources.",
        )
        missingMangaSourceIds.forEach { sourceId ->
            val name = mangaSourceMapping[sourceId] ?: sourceId.toString()
            errors.add(now to "Missing manga source: $name ($sourceId)")
        }
        missingAnimeSourceIds.forEach { sourceId ->
            val name = animeSourceMapping[sourceId] ?: sourceId.toString()
            errors.add(now to "Missing anime source: $name ($sourceId)")
        }
    }
