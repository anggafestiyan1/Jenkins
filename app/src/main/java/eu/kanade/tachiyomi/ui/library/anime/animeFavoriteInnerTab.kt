...
@Composable
fun Screen.animeFavoriteInnerTab(): TabContent {
    val screen = this
    return TabContent(
        ...
        content = { contentPadding, _ ->
            with(screen) {
                AnimeFavoriteInnerContent(contentPadding = contentPadding)
            }
        },
    )
}
...
@Composable
private fun Screen.AnimeFavoriteInnerContent(
        ...
        when {
            ...
            state.searchQuery.isNullOrEmpty() && !state.hasActiveFilters && state.isLibraryEmpty -> {
                ...
                AnimeLibraryContent(
                    ...
                    continueWatchingHistory = state.continueWatchingHistory,
                    onContinueWatchingHistoryClicked = { history ->
                        scope.launchIO {
                            val episode = screenModel.getNextEpisodeForHistory(
                                history.animeId,
                                history.episodeId,
                            )
                            if (episode != null) openEpisode(episode)
